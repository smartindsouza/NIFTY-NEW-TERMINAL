               ...
               <div className="p-12 text-center text-slate-500 flex flex-col items-center justify-center gap-2">
                ...
                <>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-4">
                     <div className="bg-[#0f1422] p-3 rounded-lg border border-white/5 text-center">
                        <p className="text-[10px] text-slate-400 uppercase font-bold mb-1">Final Regime</p>
                        <p className={`text-sm font-mono font-bold ${quantData.finalRegime === 'Bullish' ? 'text-emerald-400' : quantData.finalRegime === 'Bearish' ? 'text-rose-400' : 'text-slate-200'}`}>
                          {quantData.finalRegime}
                        </p>
                     </div>
                     <div className="bg-[#0f1422] p-3 rounded-lg border border-white/5 text-center">
                        <p className="text-[10px] text-slate-400 uppercase font-bold mb-1">Bull Score</p>
                        <p className="text-sm font-mono font-bold text-emerald-400">{quantData.bullScore}</p>
                     </div>
                     <div className="bg-[#0f1422] p-3 rounded-lg border border-white/5 text-center">
                        <p className="text-[10px] text-slate-400 uppercase font-bold mb-1">Bear Score</p>
                        <p className="text-sm font-mono font-bold text-rose-400">{quantData.bearScore}</p>
                     </div>
                     <div className="bg-[#0f1422] p-3 rounded-lg border border-white/5 text-center">
                        <p className="text-[10px] text-slate-400 uppercase font-bold mb-1">Vol Score</p>
                        <p className="text-sm font-mono font-bold text-amber-400">{quantData.volScore}</p>
                     </div>
                  </div>

                  <div className="space-y-3 pt-2 text-xs">
                    <h4 className="font-bold text-slate-300 uppercase tracking-wider mb-2">Rules Triggered</h4>
                    <div className="space-y-2">
                      {quantData.rulesTriggered?.length > 0 ? quantData.rulesTriggered.map((rule: string, idx: number) => (
                        <div key={idx} className="bg-[#0f1422] p-3 rounded-lg border border-white/5 flex gap-2 items-center">
                           <ShieldCheck className="w-3.5 h-3.5 text-indigo-400 shrink-0" />
                           <span className="font-mono text-slate-300">{rule}</span>
                        </div>
                      )) : (
                         <div className="text-slate-500 font-mono italic p-2 hidden">No significant directional rules triggered.</div>
                      )}
                    </div>
                  </div>
                </>
            ) : (
                <div className="text-slate-500 text-xs">Insufficient data to calculate rules.</div>
            )}
          </CardContent>
        </Card>
        ...
        <div className="space-y-6">
          <Card className="bg-[#121824]/60 border-white/5">
             <CardHeader className="pb-3 border-b border-white/5">
                <CardTitle className="text-sm font-semibold tracking-wide text-slate-300 flex justify-between items-center">
                  Data Context
                  <MetricSourceBadge type="LIVE" />
                </CardTitle>
             </CardHeader>
             <CardContent className="pt-4 space-y-4">
                 <div className="bg-[#0f1422] rounded-lg p-3 border border-white/5 space-y-2">
                    <p className="text-[10px] font-bold text-slate-400 uppercase">Sensors Integrated</p>
                    <div className="flex flex-wrap gap-1.5">
                       <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-white/5 text-slate-300">RSI</span>
                       <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-white/5 text-slate-300">Trend</span>
                       <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-white/5 text-slate-300">Option PCR</span>
                    </div>
                 </div>
